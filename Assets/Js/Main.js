...
function showCustomEvent(day) {
  ...
  document
    .querySelectorAll(".custom-event-buttons .event-btn")
    .forEach((button) => button.classList.remove("active"));

  // Add active class to clicked button
  document
    .querySelector(`.custom-event-buttons .event-btn:nth-child(${day})`)
    .classList.add("active");

  // Hide all event content
  document
    .querySelectorAll(".custom-event-content, .custom-event-content-2")
    .forEach((content) => {
  ...
  setTimeout(() => {
    document.getElementById(`custom-event-${day}-map`).classList.add("active");
    document
      .getElementById(`custom-event-${day}-details`)
      .classList.add("active");

    // Ensure opacity change happens after display change
    setTimeout(() => {
      document.getElementById(`custom-event-${day}-map`).style.opacity = "1";
      document.getElementById(`custom-event-${day}-details`).style.opacity =
        "1";
    }, 50);
  }, 300);
}
