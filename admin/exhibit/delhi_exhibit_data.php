...
include '../includes/db.php';
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        header("Location: ../../delhi_exhibit_reg"); 
    }
...
if (isset($_GET['delete_id'])) {
    ...
    if ($delete_id > 0) { // Validate the ID
        $stmt = $conn->prepare("DELETE FROM delhi_exhibit_data WHERE id = ?");
        $stmt->bind_param("i", $delete_id);

        if ($stmt->execute()) {
            echo "<script>console.log('Record deleted successfully');</script>";
            echo "<script>alert('Record deleted successfully');</script>";
            header("Location: ../tables/delhi_exhibit_table.php"); // Redirect to the same page
            exit();
        } else {
